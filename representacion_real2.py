...
def mutacion_gaussiana(cromosoma, sigma=0.2): #Probar con 0.05, 0.1, 0.2
    cromosoma_mutado = cromosoma.copy()

    for i in range(39):
        if random.random() < 0.1:  # probabilidad de mutar este alumno
            idx = i * 3
            genes = cromosoma_mutado[idx:idx+3]
            genes_mutados = [max(0, g + random.gauss(0, sigma)) for g in genes]
            suma = sum(genes_mutados)
            if suma > 0:
                genes_mutados = [g / suma for g in genes_mutados]
            else:
                genes_mutados = [1/3, 1/3, 1/3]
            cromosoma_mutado[idx:idx+3] = genes_mutados
...
print("REPRESENTACIÓN REAL - MODIFICACION (Mutación Gaussiana) sigma 0.1")
