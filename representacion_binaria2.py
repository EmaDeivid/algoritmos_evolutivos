# Modifica la función calcular_fitness para:
...
def calcular_fitness(cromosoma):
    ...
    for examen in ['A', 'B', 'C']:
        ...
        bajo = sum(1 for n in notas_examen if n <= 13)
        medio = sum(1 for n in notas_examen if 14 <= n <= 16)
        alto = sum(1 for n in notas_examen if n >= 17)

        categorias = [bajo > 0, medio > 0, alto > 0]
        diversidad = sum(categorias) / 3  
    ...
    recompensa_diversidad = np.mean(diversidades)  

    fitness = -desviacion_promedios - 0.1 * penalizacion_varianza + 0.5 * recompensa_diversidad
...
print("REPRESENTACIÓN BINARIA - MODIFICADA (CALCULAR FITNETS)")
